import asyncio; from lib.setup.bot import *; asyncio.run(SetupManager().run_setup())
 ...
 # Defines SetupManager and AvatarToTextArt , Installs submodules, makes a updated requirements.txt file.
 
 #######################
 
from lib.setup.terminal import AvatarToTextArt 
...
INDEX_HTML_PATH = os.path.join("bot", "website", "index.html")
STATIC_HTML_DIR = os.path.join("bot", "website")
...
async def start_web_server(port: int = 8080, ut: bool = False):
    app = web.Application()
    app.router.add_get("/", handle_index)
    app.router.add_static('/html/', path=STATIC_HTML_DIR, name='html')
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, '0.0.0.0', 0 if ut else port)
    await site.start()
    return runner, site
